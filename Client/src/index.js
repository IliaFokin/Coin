...
messageBtn.addEventListener('click', () => {
  ...
  window.document.body.classList.remove('body--stop-scroll');
})
...
function getTokenFromLocalStorage() {
  ...
  const tokenTimeDiff = Math.floor(new Date() - new Date(tokenInfo.date)) / (1000 * 60);
  if (tokenTimeDiff < 30) return tokenInfo.token;
  else {
    localStorage.removeItem('token');
    return 0;
  }
